...
void fpga_matmul(std::vector<float, aligned_allocator<float>> const &source_in1_full,
    ...
    int block_count = DATA_INNER / (BLOCK_SIZE*BLOCK_SIZE);  /////////Project

        for (int k = 0; k < DATA_INNER; k += BLOCK_SIZE) 
        {
    ...
    for (int i = 0; i < DATA1_ROWS; i += BLOCK_SIZE) 
    {

    for (int j = 0; j < DATA2_COLS; j += BLOCK_SIZE) 
    {
        sum.clear();
        ...
        // Copy over matrix blocks
            for (int x = i; x < i + BLOCK_SIZE; x++) 
            {
                ...
                {
                    source_in1.push_back(source_in1_full[x * DATA_INNER + y]);
                }
            }
            for (int x = k; x < k + BLOCK_SIZE; x++) 
            {
                ...
                {
                    source_in2.push_back(source_in2_full[x * DATA2_COLS + y]);
                }
            }	
