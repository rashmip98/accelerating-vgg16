...
v++ -t hw_emu --config q4/configs/design.cfg \
    --log_dir q4/logs \
    --report_dir q4/reports \
    --platform $AWS_PLATFORM \
    --compile --kernel multadd \
    -I q4/src -o q4/build/multadd.hw_emu.xo q4/src/multadd.cpp
echo Compiled FPGA kernel

echo Moved compile summary to reports directory
mv q4/build/*compile_summary q4/reports/multadd.hw_emu/
...
v++ -t hw_emu --config q4/configs/design.cfg \
    ...
    --report_dir q4/reports \
    --platform $AWS_PLATFORM \
    --link -o q4/build/multadd.hw_emu.xclbin \
    q4/build/multadd.hw_emu.xo
echo Linked FPGA kernel

echo Moved link summary to reports directory
mv q4/build/*link_summary q4/reports/multadd.hw_emu/
...
mv profile_summary.csv q4/reports/multadd.hw_emu/ 2>/dev/null || true
mv timeline_trace.csv q4/reports/multadd.hw_emu/ 2>/dev/null  || true
mv *.run_summary q4/reports/multadd.hw_emu/ 2>/dev/null  || true
mv profile_kernels.csv q4/reports/multadd.hw_emu/ 2>/dev/null  || true
mv timeline_kernels.csv q4/reports/multadd.hw_emu/ 2>/dev/null  || true
